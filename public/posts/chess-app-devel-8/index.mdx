...
<pstlk label="前回" to="chess-app-devel-7" />は
<tltp label="Pillow">Python の画像処理ライブラリ</tltp>
...
数学の話になりますが、三角関数の引数は
<tltp label="弧度法">度数法に 180 で割って円周率をかけたもの</tltp>
で表されるので、
...
<postimage src="sin-cos" ext="png" alt="角度と座標" />
...
<pstlk label="第6回" to="chess-app-devel-6#四角形の描画" />で
<icode>glOrtho()</icode>を使って指定しているのもワールド座標です。
...
<postimage src="world_coord" ext="png" alt="ワールド座標" />
...
そのため、<pstlk label="第4回" to="chess-app-devel-4#盤面を仮定する" />でチェック回避を考慮した`is_valid_move()`を作ったのですが、
...
<postimage src="mouse-action-result" ext="gif" alt="結果" />
...
<pstlk label="次回" to="chess-app-devel-9" /> はおまけ的な感じで、駒の動きをスムーズにしてみたいと思います。
