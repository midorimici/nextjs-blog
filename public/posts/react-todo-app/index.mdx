...
topics: [React, Recoil, MaterialUI, アプリ開発]
published: true
---

おいしそうなタイトルになってしまいました。

これまで React で<pstlk label="文字当てゲーム" to="react-mojiate-game" />や<pstlk label="タイピング練習アプリ" to="react-typing-app" />を作ってきましたが、

今回は ToDo アプリを作成しました。

<!--more-->

<tweet id="1272492269333962752" />

<sandbox name="react-mui-todo-app" link="https://codesandbox.io/s/react-mui-todo-app-gx6us?fontsize=14&hidenavigation=1&theme=dark" />
...
```tsx name=/src/components/TodoAppBar.tsx
...
<postimage src="appbar" alt="App Bar" />
...
```tsx name=/src/components/TodoList.tsx
import React from 'react';
...
```tsx name=/src/App.tsx hl_lines=4,12
import React from 'react';
...
<postimage src="not-registered" alt="タスク未登録画面" />
...
```tsx name=/src/components/RegisterDialog.tsx
import React from 'react';
...
```tsx name=/src/components/TodoList.tsx hl_lines=1,8,15-19,22,29,36-37 inline_hl=1:3-7
...
```tsx name=/src/atoms/RegisterDialogContent.tsx
import { atom } from 'recoil';
...
```tsx name=/src/components/RegisterDialogContent.tsx
import React from 'react';
...
```tsx name=/src/components/RegisterDialog.tsx hl_lines=16
...
...
```tsx name=/src/App.tsx hl_lines=2,11,16
import React from 'react';
...
```tsx name=/src/components/RegisterDialog.tsx hl_lines=17-31,47 inline_hl=47:4-9
...
...
```tsx name=/src/components/TodoTable.tsx
import React from 'react';
import { useRecoilState } from 'recoil';
...
```tsx name=/src/components/TodoList.tsx hl_lines=21-28,35,45-57,71
...
```tsx name=/src/components/TodoTable.tsx hl_lines=10-49,52-59,64-69,78-83,94
import React, { useState } from 'react';
...
...
```tsx name=/src/components/TodoTable.tsx hl_lines=20-31,46-63
...
...
`sort`メソッドは、配列を引数の関数に従って並び変える<tltp label="破壊的メソッド">操作対象を変更する</tltp>です。
