...
<pstlk label="前回" to="chess-app-devel-3" />は、ポーンについてその動きの仕組みを見たうえで、
...
<tltp label="クラス">オブジェクトの型、設計図のようなもの</tltp>
内のメソッドですね。
...
<tltp label="インスタンス（オブジェクト）">クラスをもとに作られた分身のようなもの</tltp>
を指します。
...
```py name=main.py hl_lines=3,5 inline_hl=3:6-7;5:4
class Game:
    ...
...
```py name=main.py ins=2,11-12,14,16-17,19 del=1,10,15 inline_hl=2:6-7;10:2-15;11:2-4
def isCheck(self):
...
```py name=main.py hl_lines=1,12-14 inline_hl=12:8,13-16;13:5
...
```py name=main.py ins=2,6,11,16,19 del=1,5,10,15,18 inline_hl=2:8-9;5:6-7;11:18-19;16:10-11;18:14-15
def isCheck(self, color):
...
```py name=main.py ins=2,5 del=1,4 inline_hl=1:11-12;5:22-23
def isValidMove(self, piece, startpos, endpos, Color, gameboard):
...
<details>
<summary>❓なぜ直接代入しない？</summary>
<div><br/>
`copy()`を使用しないと、盤面の更新をした際にもとの盤面も一緒に書き換えられてしまいます。

これは
<tltp label="ディクショナリ">キーと値が 1 対 1 に対応する形でまとめられた構造</tltp>
...
```py name=main.py hl_lines=19,23
def main(self):
　
    while True:
        self.printBoard()
...
<pstlk label="次回" to="chess-app-devel-5" />はチェックメイトの判定についてです。
