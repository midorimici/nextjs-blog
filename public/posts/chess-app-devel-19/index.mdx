...
<pstlk label="前回" to="chess-app-devel-18" />はキャスリングの条件判定のコードを編集しました。
...
この表示には、<pstlk label="第11回" to="chess-app-devel-11/#選択肢を表示する吹き出しを描画" />で作成した吹き出しを利用します。
...
<postimage src="castling_confirmation" alt="キャスリング確認" />
...
まず駒の移動先に移動前の駒<tltp label="インスタンス">クラスをもとに作られた実体</tltp>を移してから、もとの位置を削除しています（キングの動き）。
...
<pstlk label="次回" to="chess-app-devel-20" />はプロモーションの拡張について書きたいと思います。
