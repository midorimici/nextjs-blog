...
<pstlk label="前回" to="geister-app-dev-3" />はマウス操作で駒の初期配置を決められるようになりました。
...
```py name=main.py hl_lines=4-5, inline_hl=4:0;5:5-6
...
if __name__ == '__main__':
    ...
...
```py name=game.py hl_lines=2 inline_hl=2:6-7
...
...
<postimage src="board" alt="初期盤面" />

---

<pstlk label="次回" to="geister-app-dev-5" />は駒の移動とターン交代時の画面を用意します。
