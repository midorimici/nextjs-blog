...
<pstlk label="前回" to="chess-app-devel-19" />でフェアリーチェスへのキャスリングの拡張が完了しました。
...
<relpos link="chess-app-devel-11" />
...
これは<pstlk label="第15回" to="chess-app-devel-15/#ゲームを定義する" />ですでに完了しています。
...
```py name=main.py ins=10-14 del=6-9
# プロモーション
if self.prom:
    draw_balloon(*self.endpos)
    piece_color = self.gameboard[self.endpos].color
...
```py name=utils.py ins=2,10-11,19-22 del=1,15-18 inline_hl=2:8-11;19:6-24;20:6-24;21:2-13,18-36;22:2-13,18-36
...
```py name=main.py ins=4 del=3 inline_hl=4:6-16
# プロモーション
if self.prom:
    draw_balloon(*self.endpos)
