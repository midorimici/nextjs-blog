...
{{< pstlk 前回 react-mojiate-game >}}、React で文字当てゲームを作りました。

今回は、このようなタイピング練習アプリを作ってみました。

{{< tweet 1268135841366339585 >}}

<br/>

{{< sandbox "react-simple-typing-app" "https://codesandbox.io/s/react-simple-typing-app-s8quj?fontsize=14&hidenavigation=1&module=%2Fsrc%2FApp.tsx&view=preview" >}}
...
これらの知識がすでにある方は、最初の４項目は[読み飛ばして](#%E5%AE%9F%E8%A3%85)いただいても構いません。

{{< relpos react-mojiate-game >}}
...
```jsx name=/src/App.js del=1,3-10,13-16,19-20 inline_hl=1:4;3:2,4-5
...
```jsx name=/src/App.js ins=1,3-5,7 inline_hl=0:4;1:2,4;4:0,5-10
...
```jsx {hl_lines=[3, 4, 11], inline_hl=[0:[3], 1:["7-11"], 2:["2-4"]]}
import React from 'react';
...
[Material-UI](https://material-ui.com/ja/) は、React コンポーネントを使って{{% tltp マテリアルデザイン %}}Google が提唱したデザイン方法<br/>現実世界の物理的・物質的な性質を取り入れることで、<br/>直感的な操作性を実現する{{% /tltp %}}を実現できるライブラリです。
...
```jsx {name="/src/components/MyButton.tsx", id=6}
import React from 'react';
import Button from '@material-ui/core/Button';
...
```jsx {name="/src/components/MyButton.tsx", hl_lines=[2, "5-11", 20, 24], id=7}
import React from 'react';
...
```jsx {name="/src/App.tsx", hl_lines=[7], id=8}
import React from 'react';
...
```jsx {name="/src/App.tsx", hl_lines=[5, 7, 11], id=9}
import React, { useState } from 'react';
import MyButton from './components/MyButton';
...
```jsx {name="/src/App.tsx", hl_lines=[5, "12-14"], id=10}
import React, { useState } from 'react';
import MyButton from './components/MyButton';
...
```jsx {name="/src/App.tsx", hl_lines=[10, 13], id=11}
import React, { useState } from 'react';
import MyButton from './components/MyButton';
...
{{< img src=key-event-test ext=gif alt="React キーボードイベント テスト" >}}
...
```jsx {name="/src/App.tsx", hl_lines=[3, 8, "12-21", 26, 27], id=12}
import React, { useState } from 'react';
import MyButton from './components/MyButton';
import './App.scss';
...
{{< img src=typing-test ext=gif alt="React タイピング テスト" >}}
...
```jsx {name="/src/App.tsx", hl_lines=[9, "16-42", "49-55"], id=13}
import React, { useState } from 'react';
import MyButton from './components/MyButton';
import "./App.scss";
...
```jsx {name="/src/App.tsx", hl_lines=["2-15", 30], id=14}
...
const refresh = () => {
  // 文字の配列を取得
  let textSpans = document.querySelector("#textbox")!.children;
...
{{< video src="typing-refresh" opt=1 >}}
