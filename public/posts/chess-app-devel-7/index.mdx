...
<pstlk label="前回" to="chess-app-devel-6" />で駒を乗せるための盤面が準備できました。
...
<p>
<tltp label="[Pillow](https://pillow.readthedocs.io/en/stable/)">Python の画像処理ライブラリ</tltp>
を使って画像を処理します。
</p>
...
<tltp label="プロパティ">オブジェクトが参照できるデータ</tltp>
（`abbr`）を定義しました。
...
```py name=main.py ins=5-6,13-14,19,25-27 del=3-4,11-12 inline_hl=3:17-23;4:17-23;5:17-18;6:17-18;11:20-29;12:20-29;13:20-29;14:20-29
...
<postimage src="board1" ext="png" alt="画像透過なし" />
...
```py name=main.py hl_lines=5,6,9
def draw(self):
    '''描画コールバック'''
...
<postimage src="board2" ext="png" alt="画像透過あり" />
...
<pstlk label="次回" to="chess-app-devel-8" />はマウス操作で駒を動かせるようにしたいと思います。

この記事がお役に立てたならうれしいです☺️
