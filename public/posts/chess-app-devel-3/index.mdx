...
<pstlk label="前回" to="chess-app-devel-2" />は、先手の駒色と駒の初期配置を修正しました。
...
まず、`Pawn`<tltp label="クラス">オブジェクトの型、設計図のようなもの</tltp>
...
継承をすると、基底クラスで定義された
<tltp label="プロパティ">オブジェクトが参照できるデータ</tltp>や
<tltp label="メソッド">オブジェクトが参照できる処理</tltp>
を子クラスでも使うことができます。
...
他の駒にはないのに`Pawn`にだけ
<tltp label="コンストラクタ">インスタンス生成時に自動的に実行される初期化メソッド</tltp>
`__init__()`があります。
...
<details>
<summary>❓なぜわかる？</summary><br/>これは例えば、
...
```py name=pieces.py hl_lines=13-15
def availableMoves(self, x, y, gameboard, Color=None):
    if Color is None:
        Color = self.Color
    answers = []
...
```py name=pieces.py hl_lines=13-14
def availableMoves(self, x, y, gameboard, Color=None):
    if Color is None:
        Color = self.Color
    answers = []
...
<pstlk label="次回" to="chess-app-devel-4" />はチェックまわりについて見ていきたいと思います。
