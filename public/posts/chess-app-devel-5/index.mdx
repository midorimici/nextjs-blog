...
<pstlk label="前回" to="chess-app-devel-4" />は自らチェックされに行くような動きができないようにしました。

今回は
<tltp label="チェックメイト">キングがチェックされていて、一手でチェックから逃れることができない状態</tltp>や
<tltp label="ステイルメイト">キングがチェックされていないが、打つ手がない状態</tltp>
を判定できるようにします。
...
次回は<pstlk label="こちら" to="chess-app-devel-6" />
